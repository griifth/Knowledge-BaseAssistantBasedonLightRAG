...
from lightrag.api.rag_manager import RAGInstanceManager, RAGInstanceConfig
...
def create_app(args):
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            # Clean up database connections for all workspaces
    ...
    # Create RAG instance configuration for multi-workspace support
    rag_config = RAGInstanceConfig(
    ...
    # Create RAG instance manager for multi-workspace support
    rag_manager = RAGInstanceManager(rag_config, default_workspace=args.workspace)

    # Initialize default RAG instance
    try:
        rag = LightRAG(
            working_dir=args.working_dir,
        ...
        # Register the default instance with the manager
        rag_manager.set_default_instance(rag)
    except Exception as e:
        logger.error(f"Failed to initialize LightRAG: {e}")
        raise

    # Add routes with rag_manager for multi-workspace support
    app.include_router(
        create_document_routes(
            rag,
            doc_manager,
    ...
    async def get_status(request: Request):
        ...
        try:
            ...
            # Try to get pipeline_status, auto-initialize if workspace is not yet initialized
            try:
                ...
                logger.info(f"Auto-initializing pipeline_status for workspace: {workspace or '(default)'}")
