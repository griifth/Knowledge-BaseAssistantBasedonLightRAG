...
type Tab = 'documents' | 'knowledge-graph' | 'retrieval' | 'chat' | 'api'
  ...
  persist(
    (set) => ({
      ...
      setCurrentWorkspace: (workspace: string) => {
        ...
        // Trigger a full page data refresh when workspace changes
        // Use setTimeout to ensure the state is persisted to localStorage before reload
        setTimeout(() => {
          window.location.reload()
        }, 100)
      },

      setAvailableWorkspaces: (workspaces: string[]) => set({ availableWorkspaces: workspaces }),
    ...
    }),
    {
      ...
      migrate: (state: any, version: number) => {
        ...
        if (version < 20) {
          ...
          state.workspaceRefreshTrigger = 0
        }
        return state
      }
    }
