...
export const queryTextStream = async (
  ...
  // Add workspace header if set
  if (currentWorkspace) {
    headers['LIGHTRAG-WORKSPACE'] = currentWorkspace;
  }
 ...
 * @returns Promise with workspace list response
 */
 ...
 * @param name Workspace name (alphanumeric and underscore only)
 * @returns Promise with workspace creation response
 */
export const createWorkspace = async (name: string): Promise<WorkspaceCreateResponse> => {
 ...
 * @returns Promise with workspace deletion response
 */
export const deleteWorkspace = async (name: string): Promise<WorkspaceDeleteResponse> => {
 ...
 */
export const getWorkspaceInfo = async (name: string): Promise<WorkspaceInfo> => {
  const response = await axiosInstance.get(`/workspaces/${encodeURIComponent(name)}`)
  return response.data
}

/**
 * Get current workspace information
 * @returns Promise with current workspace info
 */
export const getCurrentWorkspace = async (): Promise<{ status: string; workspace: string; info: WorkspaceInfo }> => {
  const response = await axiosInstance.get('/workspaces/current')
  return response.data
}
